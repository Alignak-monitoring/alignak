...
The **check_hpjd** plugin (which is part of the standard Nagios/Alignak plugins distribution) allows you to monitor the status of JetDirect-capable printers which have "SNMP" enabled. The plugin is capable of detecting the following printer states:
...
.. image:: /_static/images//official/images/monitoring-printers-alignak.png
   :scale: 90 %

Monitoring the status of a networked printer is pretty simple. JetDirect-enabled printers usually have "SNMP" enabled, which allows Alignak to monitor their status using the **check_hpjd** plugin.
...
Declare your new printer in Alignak 
====================================
...
::
  ...
  linux:~ # vi /etc/alignak/hosts/printer-1.cfg
  
Or Windows:
  
::

  c:\ wordpad   c:\alignak\etc\hosts\printer-1.cfg
...
You're done with modifying the Alignak configuration, so you will need to :ref:`verify your configuration files <runningalignak/verifyconfig>` and :ref:`restart Alignak <runningalignak/startstop>`.
