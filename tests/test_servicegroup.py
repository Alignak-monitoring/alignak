...
class TestServiceGroup(AlignakTest):
    ...
    def test_look_for_alias(self):
        ...
        sg = self._arbiter.conf.servicegroups.find_by_name("NOALIAS")
    ...
    def test_servicegroup_members(self):
        ...
        assert len(self._arbiter.conf.servicegroups.get_members_of_group("allservices_and_groups")) == 1
    ...
    def test_members_servicegroup(self):
        ...
        for service_id in sg.members:
            service = self._arbiter.conf.services[service_id]
            ...
            if service.get_name() == 'test_ok_0':
                ...
                for group_id in service.servicegroups:
                    group = self._arbiter.conf.servicegroups[group_id]
                    print(("Group: %s" % group))
    ...
    def test_servicegroup_with_no_service(self):
        ...
        sg = self._arbiter.conf.servicegroups.find_by_name("void")
        assert isinstance(sg, Servicegroup)
    ...
    def test_servicegroup_with_space(self):
        ...
        self.setup_with_file('cfg/cfg_default.cfg')
        assert self.conf_is_correct
        self.nb_servicegroups = len(self._arbiter.conf.servicegroups)

        self.setup_with_file('cfg/servicegroup/alignak_servicegroup_with_space.cfg')
        assert self.conf_is_correct

        # Two more groups than the default configuration
        assert len(self._arbiter.conf.servicegroups) == self.nb_servicegroups + 2

        assert self._arbiter.conf.servicegroups.find_by_name("test_With Spaces").get_name() == \
            "test_With Spaces"
        assert self._arbiter.conf.servicegroups.get_members_of_group(
            "test_With Spaces") is not []

        assert self._arbiter.conf.servicegroups.find_by_name("test_With another Spaces").get_name() == \
    ...
    def test_servicegroups_generated(self):
        ...
        for name in ["MYSVCGP", "MYSVCGP2", "MYSVCGP3", "MYSVCGP4"]:
            sg = self._arbiter.conf.servicegroups.find_by_name(name)
            assert sg is not None
            sgs.append(sg)

        svc3 = self._arbiter.conf.services.find_srv_by_name_and_hostname("fake host", "fake svc3")
        svc4 = self._arbiter.conf.services.find_srv_by_name_and_hostname("fake host", "fake svc4")
