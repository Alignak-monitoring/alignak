...
class TestServiceGroup(AlignakTest):
    ...
    def test_look_for_alias(self):
        ...
        sg = self.schedulers[0].sched.servicegroups.find_by_name("NOALIAS")
    ...
    def test_members_servicegroup(self):
        ...
        self.assertEqual(
            len(self.schedulers[0].sched.servicegroups.get_members_by_name(
        ...
        for service_id in sg.members:
            service = self.schedulers[0].sched.services[service_id]
            ...
            if service.get_name() == 'test_ok_0':
                ...
                for group_id in service.servicegroups:
                    group = self.schedulers[0].sched.servicegroups[group_id]
                    print("Group: %s" % group)
    ...
    def test_servicegroup_with_no_service(self):
        ...
        sg = self.schedulers[0].sched.servicegroups.find_by_name("void")
        self.assertIsInstance(sg, Servicegroup)
        self.assertEqual(sg.get_name(), "void")

        self.assertEqual(
            len(self.schedulers[0].sched.servicegroups.get_members_by_name("void")),
            0
        )
    ...
    def test_servicegroup_with_space(self):
        ...
        self.assertEqual(
            len(self.schedulers[0].sched.servicegroups), self.nb_servicegroups + 2
        )

        self.assertEqual(
            self.schedulers[0].sched.servicegroups.find_by_name("test_With Spaces").get_name(),
            "test_With Spaces"
        )
        self.assertIsNot(
            self.schedulers[0].sched.servicegroups.get_members_by_name(
                "test_With Spaces"
            ),
            []
        )
        ...
        self.assertEqual(
            self.schedulers[0].sched.servicegroups.find_by_name("test_With another Spaces").get_name(),
