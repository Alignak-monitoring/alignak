...
class TestContactGroup(AlignakTest):
    ...
    def test_look_for_alias(self):
        ...
        cg = self.schedulers[0].sched.contactgroups.find_by_name("NOALIAS")
    ...
    def test_contactgroup_members(self):
        ...
        cg = self.schedulers[0].sched.contactgroups.find_by_name("allcontacts_and_groups")
        self.assertIsInstance(cg, Contactgroup)
    ...
    def test_members_contactgroup(self):
        ...
        self.assertEqual(
            len(self.schedulers[0].sched.contactgroups.get_members_by_name(
        ...
        for contact_id in cg.members:
            contact = self.schedulers[0].sched.contacts[contact_id]
            ...
            if contact.get_name() == 'test_ok_0':
                ...
                for group_id in contact.contactgroups:
                    group = self.schedulers[0].sched.contactgroups[group_id]
                    print("Group: %s" % group)
    ...
    def test_contactgroup_with_no_contact(self):
        ...
        self.assertEqual(
            len(self.schedulers[0].sched.contactgroups),
            3
        )

        for group in self.schedulers[0].sched.contactgroups:
        ...
        cg = self.schedulers[0].sched.contactgroups.find_by_name("void")
        print("cg: %s" % cg)
        self.assertIsInstance(cg, Contactgroup)
        self.assertEqual(cg.get_name(), "void")

        self.assertEqual(
            len(self.schedulers[0].sched.contactgroups.get_members_by_name("void")),
            0
        )
    ...
    def test_contactgroup_with_space(self):
        ...
        self.setup_with_file('cfg/cfg_default.cfg')
        self.assertTrue(self.schedulers[0].conf.conf_is_correct)
        self.nb_contactgroups = len(self.schedulers[0].sched.contactgroups)
        ...
        self.assertEqual(
            len(self.schedulers[0].sched.contactgroups), self.nb_contactgroups + 1
        )

        self.assertEqual(
            self.schedulers[0].sched.contactgroups.find_by_name("test_With Spaces").get_name(),
            "test_With Spaces"
        )
        self.assertIsNot(
            self.schedulers[0].sched.contactgroups.get_members_by_name(
                "test_With Spaces"
            ),
            []
        )
    ...
    def _dump_host(self, h):
        ...
        for c in h.contacts:
            print "->", self.schedulers[0].sched.contacts[c].get_name()
    ...
    def test_contactgroups_plus_inheritance(self):
        ...
        host0 = self.schedulers[0].sched.hosts.find_by_name("test_host_0")
        ...
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host0.contacts]
        )
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host0.contacts]
        ...
        host2 = self.schedulers[0].sched.hosts.find_by_name("test_host_2")
        self._dump_host(host2)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host2.contacts]
        )

        host3 = self.schedulers[0].sched.hosts.find_by_name("test_host_3")
        self._dump_host(host3)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host3.contacts]
        )
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host3.contacts]
        ...
        host4 = self.schedulers[0].sched.hosts.find_by_name("test_host_4")
        self._dump_host(host4)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host4.contacts]
        )

        host5 = self.schedulers[0].sched.hosts.find_by_name("test_host_5")
        self._dump_host(host5)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host5.contacts]
        )
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host5.contacts]
        ...
        host6 = self.schedulers[0].sched.hosts.find_by_name("test_host_6")
        self._dump_host(host6)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host6.contacts]
        )
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in host6.contacts]
        ...
        svc1 = self.schedulers[0].sched.services.find_srv_by_name_and_hostname(
            "test_host_0", "svc_tmplA"
        )
        self._dump_svc(svc1)
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in svc1.contacts]
        )
        ...
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in svc2.contacts]
        )

        svc3 = self.schedulers[0].sched.services.find_srv_by_name_and_hostname(
            "test_host_0", "svc_tmplA_tmplB"
        )
        self.assertIn(
            "test_contact_1",
            [self.schedulers[0].sched.contacts[c].get_name() for c in svc3.contacts]
        )
        self.assertIn(
            "test_contact_2",
            [self.schedulers[0].sched.contacts[c].get_name() for c in svc3.contacts]
