...
class TestBusinessCorrelator(AlignakTest):
    ...
    def test_simple_or_business_correlator(self):
        ...
        router = self.scheduler.sched.hosts.find_by_name("test_router_0")
        ...
        svc_bd1 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "db1")
        self.assertEqual(False, svc_bd1.got_business_rule)
        self.assertIs(None, svc_bd1.business_rule)
        svc_bd2 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "db2")
        self.assertEqual(False, svc_bd2.got_business_rule)
        self.assertIs(None, svc_bd2.business_rule)
        svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Simple_Or")
    ...
    def test_simple_and_business_correlator(self):
        ...
        self.assertEqual(False, svc_bd2.got_business_rule)
        self.assertIs(None, svc_bd2.business_rule)
        svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Simple_And")
        self.assertEqual(True, svc_cor.got_business_rule)
        self.assertIsNot(svc_cor.business_rule, None)
        bp_rule = svc_cor.business_rule
        self.assertEqual('&', bp_rule.operand)
    ...
    def run_simple_1of_business_correlator(self, with_pct=False, with_neg=False):
        ...
        if with_pct is True:
            if with_neg is True:
                svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname(
    ...
    def test_simple_or_business_correlator_with_schedule(self):
        ...
        self.scheduler.sched.add(svc_cor.launch_check(now-1, self.scheduler.sched.hosts, self.scheduler.sched.services, self.scheduler.sched.timeperiods,
                             self.scheduler.sched.macromodulations, self.scheduler.sched.checkmodulations, self.scheduler.sched.checks))
        c = svc_cor.actions[0]
        ...
        for i in svc_bd2.impacts:
            print self.scheduler.sched.find_item_by_id(i).get_name()
    ...
    def test_full_erp_rule_with_schedule(self):
        ...
        self.assertEqual(False, svc_bd2.got_business_rule)
        self.assertIs(None, svc_bd2.business_rule)
        svc_web1 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "web1")
        self.assertEqual(False, svc_web1.got_business_rule)
        self.assertIs(None, svc_web1.business_rule)
        svc_web2 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "web2")
        self.assertEqual(False, svc_web2.got_business_rule)
        self.assertIs(None, svc_web2.business_rule)
        svc_lvs1 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "lvs1")
        self.assertEqual(False, svc_lvs1.got_business_rule)
        self.assertIs(None, svc_lvs1.business_rule)
        svc_lvs2 = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "lvs2")
        self.assertEqual(False, svc_lvs2.got_business_rule)
        self.assertIs(None, svc_lvs2.business_rule)
        svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "ERP")
        self.assertEqual(True, svc_cor.got_business_rule)
        self.assertIsNot(svc_cor.business_rule, None)
        bp_rule = svc_cor.business_rule
        self.assertEqual('&', bp_rule.operand)
    ...
    def run_complex_ABCof_business_correlator(self, with_pct=False):
        ...
        A = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "A")
        self.assertEqual(False, A.got_business_rule)
        self.assertIs(None, A.business_rule)
        B = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "B")
        self.assertEqual(False, B.got_business_rule)
        self.assertIs(None, B.business_rule)
        C = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "C")
        self.assertEqual(False, C.got_business_rule)
        self.assertIs(None, C.business_rule)
        D = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "D")
        self.assertEqual(False, D.got_business_rule)
        self.assertIs(None, D.business_rule)
        E = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "E")
        self.assertEqual(False, E.got_business_rule)
        self.assertIs(None, E.business_rule)

        if with_pct == False:
            svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Complex_ABCOf")
        else:
            svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Complex_ABCOf_pct")
        self.assertEqual(True, svc_cor.got_business_rule)
        self.assertIsNot(svc_cor.business_rule, None)
        bp_rule = svc_cor.business_rule
        self.assertEqual('of:', bp_rule.operand)
    ...
    def test_simple_and_not_business_correlator(self):
        ...
        self.assertEqual(False, svc_bd2.got_business_rule)
        self.assertIs(None, svc_bd2.business_rule)
        svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Simple_And_not")
        self.assertEqual(True, svc_cor.got_business_rule)
        self.assertIsNot(svc_cor.business_rule, None)
        bp_rule = svc_cor.business_rule
        self.assertEqual('&', bp_rule.operand)
    ...
    def test_multi_layers(self):
        ...
        self.assertEqual(False, svc_bd2.got_business_rule)
        self.assertIs(None, svc_bd2.business_rule)
        svc_cor = self.scheduler.sched.services.find_srv_by_name_and_hostname("test_host_0", "Multi_levels")
        self.assertEqual(True, svc_cor.got_business_rule)
        self.assertIsNot(svc_cor.business_rule, None)
        bp_rule = svc_cor.business_rule
        self.assertEqual('&', bp_rule.operand)
        ...
        for p in svc_cor.source_problems:
            print self.scheduler.sched.find_item_by_id(p).get_full_name()
    ...
    def test_darthelmet_rule(self):
        ...
        print "Get the hosts and services"
        now = time.time()
        host = self.scheduler.sched.hosts.find_by_name("test_darthelmet")
        host.checks_in_progress = []
        host.act_depend_of = []  # ignore the router
        A = self.scheduler.sched.hosts.find_by_name("test_darthelmet_A")
        B = self.scheduler.sched.hosts.find_by_name("test_darthelmet_B")
