...
class TestExternalCommands(AlignakTest):
    ...
    def test_external_commands(self):
        ...
        router.active_checks_enabled = False
        ...
        excmd = '[%d] PROCESS_HOST_CHECK_RESULT;test_router_0;2;Router is Down' % very_past
        self.schedulers[0].sched.run_external_command(excmd)
        self.external_command_loop()
        self.assertEqual('DOWN', router.state)
        self.assertEqual('Router is Down', router.output)
    ...
    def test_external_commands_only_passively_checked(self):
        now = time.time()
        # delete schedule
        # del self.schedulers[0].sched.recurrent_works[1]
        ...
        # ACK SERVICE
        excmd = '[%d] ACKNOWLEDGE_SVC_PROBLEM;test_host_0;test_ok_0;2;1;1;Big brother;Acknowledge service' % int(
            time.time())
        self.schedulers[0].sched.run_external_command(excmd)
        self.external_command_loop(False)
        self.assertEqual('WARNING', svc.state)
        ...
        # Now with crappy characters, like é
        host = self.schedulers[0].sched.hosts.find_by_name("test_router_0")
        excmd = '[%d] PROCESS_HOST_CHECK_RESULT;test_router_0;2;Bob got a crappy character  é   and so is not not happy|rtt=9999' % int(
            time.time())
        self.schedulers[0].sched.run_external_command(excmd)
        self.external_command_loop()
        self.assertEqual('DOWN', host.state)
